...
export default function Seller (){
    ...
    const fetchPreviousDayOrder = () => {
        ...
        axios.post(`http://${backendPort}/api/v1/reconciliation/search`, null, {
            ...
            .then(response => {
                ...
                setOrderInfo(data);
                console.log("sellerfetch",orderInfo);
            })
            .catch(error => {
                console.error(error);
            });
    ...
    useEffect(() => {
        ...
        // 订阅修改消息
        const mySub = PubSub.subscribe('卖方账户余额', correctSellerBalance);
        console.log("订阅成功")
    }, []);
    ...
    const postNewBalance = () => {
        ...
        axios.post(`http://${backendPort}/api/v1/reconciliation/updateBalance`, null, {
            params: {
                order_id: order_id,
                object: 1,
                balance: orderInfo.seller_balance
            },
            headers: {
                'Authorization': `Bearer ${Token}`,
                'Content-Type': 'application/json; charset=ISO-8859-1'
    ...
    const correctSellerBalance = (msgName, newData) => {
        const newOrder = orderInfo;
        newOrder.seller_balance = newData.newBalance;
        ...
        return(
            <div>
                ...
                <div className="input-body">
                    <ReadInput label={orderInfo.seller_name} name="卖方用户名"/>
                </div>
                <div className="input-body">
                ...
                {orderInfo.state === 0?
                        <ReadInput label={orderInfo.seller_balance} name="卖方账户余额"/>:
                        <WriteInput label={orderInfo.seller_balance} name="卖方账户余额"/>}
                </div>
            </div>
        )
    }
